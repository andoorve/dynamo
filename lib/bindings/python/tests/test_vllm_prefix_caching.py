...
def test_decode():
    ...
    assert (
        manager.single_type_manager.req_to_blocks[req0.request_id][-1].block_hash
        is None
    )
    ...
    assert (
        manager.single_type_manager.req_to_blocks[req0.request_id][-2].block_hash
        is not None
    )
    assert (
        manager.single_type_manager.req_to_blocks[req0.request_id][-1].block_hash
...
def test_hash_block_correct_reuse():
    ...
    assert num_computed_tokens == 0
    blocks = manager.allocate_slots(
        req, num_tokens - 1, len(computed_blocks.blocks) * 16, computed_blocks
    )
    assert len(blocks.blocks) == 1

    assert manager.block_pool.blocks[blocks.blocks[0].block_id].block_hash is None
...
def test_basic_prefix_caching_disabled():
    ...
    assert num_computed_tokens == 0
    blocks = manager.allocate_slots(
        req1, 10, len(computed_blocks.blocks) * 16, computed_blocks
    )
    ...
    assert num_computed_tokens == 0
    blocks = manager.allocate_slots(
        req2, 16, len(computed_blocks.blocks) * 16, computed_blocks
    )
    ...
    assert num_computed_tokens == 0
    blocks = manager.allocate_slots(
        req3, 4, len(computed_blocks.blocks) * 16, computed_blocks
    )
    assert not blocks
...
def test_prefill_not_enough_free_blocks_with_computed_blocks():
    ...
    assert num_computed_tokens == 3 * 16
    manager.allocate_slots(req1, 48, len(computed_blocks.blocks) * 16, computed_blocks)
...
def test_reset_prefix_cache():
    ...
    assert len(computed_blocks.blocks) == 3
    blocks = manager.allocate_slots(
        req1, 7, len(computed_blocks.blocks) * 16, computed_blocks
    )
    assert blocks.get_block_ids() == [[5]]
...
def test_prefix_cache_stats_disabled():
    ...
    assert not computed_blocks.blocks
    assert num_computed_tokens == 0
    manager.allocate_slots(req, 16, len(computed_blocks.blocks) * 16, computed_blocks)
...
def test_eagle_enabled_removes_last_block():
    ...
    manager.allocate_slots(
        req, len(token_ids), len(computed_blocks.blocks) * 16, computed_blocks
    )
    manager.free(req)
